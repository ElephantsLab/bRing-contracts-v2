...
describe("compare calculations with the withdrawal of stakes at the same time", () => {
    contract("with the same stake amount", async accounts => {
    ...
    contract("with different stake amount", async accounts => {
        const [ deployer, firstAddr, secondAddr, thirdAddr, fourthAddr, fifthAddr ] = accounts;
        ...
        let firstStakeAmount = 10000;
        let secondStakeAmount = 1000;
        let thirdStakeAmount = 500;
        let fourthStakeAmount = 2000;
        let fifthStakeAmount = 50000;
        ...
        it("users should have firstToken in their addresses", async () => {
            let users = [firstAddr, secondAddr, thirdAddr, fourthAddr, fifthAddr];
            let usersStakeAmount = [firstStakeAmount, secondStakeAmount, thirdStakeAmount, fourthStakeAmount, fifthStakeAmount];
            let userBalance;
            ...
            for(let i = 0; i < users.length; i++) {
                await firstToken.transfer(users[i], (new BN(usersStakeAmount[i])).mul(tokenbits), { from: deployer });
                userBalance = await firstToken.balanceOf.call(users[i]);
                assert.equal(userBalance.valueOf(), Number((new BN(usersStakeAmount[i])).mul(tokenbits)), `${users[i]} user tokens balance is wrong`);
            }
        })
        ...
        //         poolData = await bRingFarming.pools(firstTokenAddress, { from: deployer });
        //         totalStaked = Number(poolData.totalStaked);
        //         rewardWithoutMultiplier = usersStakeAmount[i] * (( poolEndTime - stakeStartTime ) / totalStaked) * rewardRatePerTime;
        ...
        //         console.log(`-- ${i+1} user --`);
        //         console.log("balance: ", userBalance - usersStakeAmount[i]);
        //         console.log("--------");
        //     }
        // })
